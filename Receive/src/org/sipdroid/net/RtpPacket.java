...
public class RtpPacket {
	...
	public String byteToString(byte[] source) {
		...
		for (int i = 0; i < source.length; i++) {
			String sequence=Integer.toBinaryString((source[i] & 0xff)+ 0x100)
					.substring(1);
			sb.append(sequence);
			sb.append("*");
		}
	...
	public String byteToHex(byte[] source){
		StringBuilder sb=new StringBuilder();
		for(int i=0;i<source.length;i++){
			String sequence=Integer.toHexString(source[i]& 0xff);
			sb.append(sequence);
		}
		return sb.toString();
	}
	...
	public String toString() {
		return byteToString(packet) + "/n" + packet_len;
	}
	...
	/** Gets int value */
	/*
	 * private static int getInt(byte b) { return ((int) b + 256) % 256; }
	 */
