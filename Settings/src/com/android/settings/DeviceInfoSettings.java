...
public class DeviceInfoSettings extends RestrictedSettingsFragment implements OnPreferenceChangeListener{
    ...
    private static final String KEY_DEVICE_NAME = "device_name_preference";
    ...
    private static final String SETTING_PREFERENCE_FILE = "com.android.settings_preferences";
    ...
    public void onCreate(Bundle icicle) {
        ...
        //setValueSummary(KEY_EQUIPMENT_ID, getEquipmentId());
        setStringSummary(KEY_DEVICE_MODEL, Build.MODEL);
        setStringSummary(KEY_BUILD_NUMBER, Build.DISPLAY);
        findPreference(KEY_BUILD_NUMBER).setEnabled(true);
        findPreference(KEY_KERNEL_VERSION).setSummary(getFormattedKernelVersion());
        findPreference(KEY_EQUIPMENT_ID).setSummary(getEquipmentId());
        ...
        ListPreference hostName = (ListPreference) findPreference(KEY_DEVICE_NAME);
        hostName.setOnPreferenceChangeListener(this);

        String devicename =  Settings.Global.getString(getContentResolver(), Global.DEVICE_NAME);
        if (null == devicename) {
            StringBuffer sb = new StringBuffer();
            sb.append(DEFAULT_DEVICE_NAME).append("_").append(getEquipmentId().substring(8,12));
            devicename = sb.toString();
        }
        String[] values = getResources().getStringArray(R.array.device_name_entries_value);
        int value = 0;
        for (int i = 0; i < values.length; i++) {
            if(devicename.contains(values[i])){
                value = i;
                break;
            }
        }
        hostName.setSummary(devicename);
        hostName.setValueIndex(value);
    }
    ...
    {
        if(preference.getKey().equals(KEY_DEVICE_NAME))
        {
            StringBuffer sb = new StringBuffer();
            sb.append(newValue.toString()).append("_").append(getEquipmentId().substring(8,12));
            Context context = getActivity();
            Settings.Global.putString(context.getContentResolver(), Global.DEVICE_NAME, sb.toString());
