...
public class Settings extends PreferenceActivity implements ButtonBarHandler,
	...
	// BEGIN: The last open fragement
	private String lastHeaderFragement = "com.android.settings.wifi.WifiSettings";
	...
	private int[] SETTINGS_FOR_RESTRICTED = {
			...
			R.id.personal_section, R.id.location_settings,
			R.id.security_settings, R.id.language_settings, R.id.user_settings,
			R.id.account_settings, R.id.account_add, R.id.system_section,
			R.id.date_time_settings, R.id.standby_settings,
			R.id.about_settings, R.id.accessibility_settings,
			R.id.print_settings, R.id.nfc_payment_settings, R.id.home_settings };
	...
	protected void onCreate(Bundle savedInstanceState) {
		if (getIntent().hasExtra(EXTRA_UI_OPTIONS)) {
			getWindow().setUiOptions(
					getIntent().getIntExtra(EXTRA_UI_OPTIONS, 0));
		}
	...
	private final String PACKAGE_NAME = "com.android.factorytest";
	private final String ACTIVITY_NAME = "com.android.factorytest.factorytestActivity";
	private int keyTemp = 0;

	@Override
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (KeyEvent.KEYCODE_5 == keyCode) {
			keyTemp = 1;
			...
			if (keyTemp == 3) {
				try {
					...
					ComponentName name = new ComponentName(PACKAGE_NAME,
							ACTIVITY_NAME);
	...
	private static final String[] ENTRY_FRAGMENTS = {
			WirelessSettings.class.getName(), WifiSettings.class.getName(),
			AdvancedWifiSettings.class.getName(),
			BluetoothSettings.class.getName(), TetherSettings.class.getName(),
			WifiP2pSettings.class.getName(), VpnSettings.class.getName(),
			DateTimeSettings.class.getName(), LocalePicker.class.getName(),
			InputMethodAndLanguageSettings.class.getName(),
			SpellCheckersSettings.class.getName(),
			UserDictionaryList.class.getName(),
			UserDictionarySettings.class.getName(),
			SoundSettings.class.getName(), DisplaySettings.class.getName(),
			DeviceInfoSettings.class.getName(),
			ManageApplications.class.getName(), ProcessStatsUi.class.getName(),
			NotificationStation.class.getName(),
			LocationSettings.class.getName(), SecuritySettings.class.getName(),
			PrivacySettings.class.getName(),
			DeviceAdminSettings.class.getName(),
			AccessibilitySettings.class.getName(),
			ToggleCaptioningPreferenceFragment.class.getName(),
			TextToSpeechSettings.class.getName(), Memory.class.getName(),
			DevelopmentSettings.class.getName(), UsbSettings.class.getName(),
			AndroidBeam.class.getName(), WifiDisplaySettings.class.getName(),
			PowerUsageSummary.class.getName(),
			AccountSyncSettings.class.getName(),
			CryptKeeperSettings.class.getName(),
			DataUsageSummary.class.getName(), DreamSettings.class.getName(),
	...
	// BEGIN: Solve the remote control can't switch wifi, network, and bluetooth
	@Override
	protected void onListItemClick(ListView lv, View v, int position, long id) {
		super.onListItemClick(lv, v, position, id);
		Header header = getHeaders().get(position);
		lv.setFocusable(true);
		int headerType = HeaderAdapter.getHeaderType(header);
		if (headerType == HeaderAdapter.HEADER_TYPE_SWITCH
				&& header.fragment.equals(lastHeaderFragement)) {// HEADER_TYPE_SWITCH
			// switchToChange(header);
		}
		lastHeaderFragement = header.fragment;
	}

	protected void switchToChange(Header header) {
		if (header.id == R.id.wifi_settings) {
			WifiEnabler mWifiEnabler = mHeaderAdapter.getWifiEnabler();
			Log.d(LOG_TAG, "wifi_settings");
			boolean checked = !mWifiEnabler.getSwitch().isChecked();
			if (mWifiEnabler.getSwitch().isEnabled()) {
				mWifiEnabler.getSwitch().setChecked(checked);
				// mWifiEnabler.onCheckedChanged(mWifiEnabler.getSwitch(),checked);
			}
		} else if (header.id == R.id.ethernet_settings) {
			Log.d(LOG_TAG, "ethernet_settings");
			EthernetEnabler mEthernetEnabler = mHeaderAdapter
					.getEthernetEnabler();
			boolean checked = !mEthernetEnabler.getSwitch().isChecked();
			mEthernetEnabler.getSwitch().setChecked(checked);
		} else if (header.id == R.id.bluetooth_settings) {
			Log.d(LOG_TAG, "bluetooth_settings");
			BluetoothEnabler mBluetoothEnabler = mHeaderAdapter
					.getBluetoothEnabler();
			boolean checked = !mBluetoothEnabler.getSwitch().isChecked();
			mBluetoothEnabler.getSwitch().setChecked(checked);
		}
	}

	// END: The remote control switch processing
	/**
	...
	public Intent onBuildStartFragmentIntent(String fragmentName, Bundle args,
		...
		if (WifiSettings.class.getName().equals(fragmentName)
				...
				|| PrintServiceSettingsFragment.class.getName().equals(
						fragmentName)) {
			intent.putExtra(EXTRA_UI_OPTIONS,
	...
	private void updateHeaderList(List<Header> target) {
        ...
        while (i < target.size()) {
            ...
            else if (id == R.id.external_storage_settings) {
                if (SystemProperties.get("ro.defaultStorage.enable", "false").equals("false")) {
                    target.remove(i);
                }
            } else if (id == R.id.third_gen_settings) {
                // Remove 3G Settings if 3G service is not available.
                if(!(SystemProperties.get("ro.hs.network.enable3g").equals("enable"))) {
                    target.remove(i);
                }
            }
        ...
        */
           ...
           /**
            *  @author xll
            *  remove wireless and ethernet for JSDX
            *  **/
            if(id==R.id.wireless_section||id==R.id.wifi_settings||id==R.id.ethernet_settings){
                target.remove(i);
            }
	...
	private boolean updateHomeSettingHeaders(Header header) {
		...
		try {
			...
			if (homeApps.size() < 2) {
				...
				header.fragmentArguments.putBoolean(
	...
	private static class HeaderAdapter extends ArrayAdapter<Header> {
		...
		static int getHeaderType(Header header) {
			...
			// else if (header.id == R.id.wifi_settings || header.id ==
			// R.id.bluetooth_settings) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				case HEADER_TYPE_BUTTON:
					view = mInflater.inflate(
							R.layout.preference_header_button_item, parent,
							false);
					holder.icon = (ImageView) view.findViewById(R.id.icon);
					holder.title = (TextView) view
							.findViewById(com.android.internal.R.id.title);
					holder.summary = (TextView) view
							.findViewById(com.android.internal.R.id.summary);
					holder.button_ = (ImageButton) view
							.findViewById(R.id.buttonWidget);
			...
			case HEADER_TYPE_SWITCH:
				...
				if (header.id == R.id.wifi_settings) {
					...
					mEthernetEnabler
							.setSwitch(holder.switch_, mEthernetEnabler);
				} else {
			...
			case HEADER_TYPE_BUTTON:
				if (header.id == R.id.security_settings) {
					boolean hasCert = DevicePolicyManager
							.hasAnyCaCertsInstalled();
					if (hasCert) {
						holder.button_.setVisibility(View.VISIBLE);
						holder.divider_.setVisibility(View.VISIBLE);
						boolean isManaged = mDevicePolicyManager
								.getDeviceOwner() != null;
						if (isManaged) {
							holder.button_
									.setImageResource(R.drawable.ic_settings_about);
						} else {
							holder.button_
		...
		private void updateCommonHeaderView(Header header,
				HeaderViewHolder holder) {
			if (header.extras != null
				...
				Drawable icon = mAuthHelper.getDrawableForType(getContext(),
						accType);
		...
		public EthernetEnabler getEthernetEnabler() {
			return mEthernetEnabler;
		}
	...
	public static class MediaSettingsActivity extends Settings { /* empty */
	}

	public static class NetSambaSettingsActivity extends Settings { /* empty */
	}

	public static class UpgradeSettingsActivity extends Settings { /* empty */
	}

	public static class ExternalStorageActivity extends Settings { /* empty */
	}

	public static class StandbySettingsActivity extends Settings { /* empty */
	}
}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             