...
public class RingerVolumePreference extends MyVolumePreference {
		...
		private void updateSlidersAndMutedStates() {
			...
			if (mCheckBoxes[i] != null) {
				if (streamType == AudioManager.STREAM_RING
					...
					mCheckBoxes[i]
							.setImageResource(com.android.internal.R.drawable.ic_audio_ring_notif_vibrate);
				} else {
				    boolean isMuted = muted || mAudioManager.getStreamVolume(streamType)<=0;
                    mCheckBoxes[i].setImageResource(isMuted ? SEEKBAR_MUTED_RES_ID[i]
                                    : SEEKBAR_UNMUTED_RES_ID[i]);
				}
			}
			if (mSeekBars[i] != null) {
				final int volume = muted ? mAudioManager
						.getLastAudibleStreamVolume(streamType) : mAudioManager
	...
	protected void onBindDialogView(View view) {
		...
		for (int i = 0; i < SEEKBAR_ID.length; i++) {
			...
			if (SEEKBAR_TYPE[i] == AudioManager.STREAM_MUSIC) {
				mSeekBarVolumizer[i] = new SeekBarVolumizer(getContext(),
						seekBar, SEEKBAR_TYPE[i],
						getMediaVolumeUri(getContext()));
			} else {
				mSeekBarVolumizer[i] = new SeekBarVolumizer(getContext(),
						seekBar, SEEKBAR_TYPE[i]);
			}
		...
		final int silentableStreams = System
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (AudioManager.RINGER_MODE_CHANGED_ACTION.equals(action)) {
						mHandler.sendMessage(mHandler.obtainMessage(
								MSG_RINGER_MODE_CHANGED, intent.getIntExtra(
										AudioManager.EXTRA_RINGER_MODE, -1), 0));
					}
				}
			};
