...
public class TetherSettings extends SettingsPreferenceFragment
	...
	private String[] mHiddenssidString;
    ...
    private void initWifiTethering() {
        ...
        mHiddenssidString= this.getResources().getStringArray(R.array.wifi_hiddenssids);
        mCreateNetwork = findPreference(WIFI_AP_SSID_AND_SECURITY);

	WifiApDialog.HiddenSSID_ON_OFF=SystemProperties.get("persist.sys.settings.ssidonoff");
        if (WifiApDialog.HiddenSSID_ON_OFF.equals("true")) {
           mHiddenSSID=true;
        }else {
            mHiddenSSID=false;
        }
		boolean isChannel=false;
        if(SystemProperties.get("persist.sys.settings.channels")==null || SystemProperties.get("persist.sys.settings.channels").equals("")){
			mWifiConfig.channel = 1;
			isChannel=false;
		}else{
			mWifiConfig.channel = Integer.parseInt(SystemProperties.get("persist.sys.settings.channels"));
			isChannel=true;
		}
        if (mWifiConfig == null) {
            ...
            mCreateNetwork.setSummary(String.format(activity.getString(CONFIG_SUBTEXT),
                    s, mSecurityType[WifiApDialog.OPEN_INDEX],mHiddenSSID ? mHiddenssidString[0]:mHiddenssidString[1],isChannel ? SystemProperties.get("persist.sys.settings.channels"):"1"));
        } else {
            int index = WifiApDialog.getSecurityTypeIndex(mWifiConfig);
            mCreateNetwork.setSummary(String.format(activity.getString(CONFIG_SUBTEXT),
                    mWifiConfig.SSID,
                    mSecurityType[index],mHiddenSSID ? mHiddenssidString[0]:mHiddenssidString[1] ,isChannel ? SystemProperties.get("persist.sys.settings.channels"):"1"));
        }
    }
            ...
            if (mWifiConfig != null) {
               ...
               if (WifiApDialog.HiddenSSID_ON_OFF.equals("true")) {
                ...
                mCreateNetwork.setSummary(String.format(getActivity().getString(CONFIG_SUBTEXT),
                        mWifiConfig.SSID,
                        mSecurityType[index],mHiddenSSID ? mHiddenssidString[0]:mHiddenssidString[1],SystemProperties.get("persist.sys.settings.channels")));
            }
        }
    }
